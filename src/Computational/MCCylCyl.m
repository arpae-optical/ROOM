...
% \___/         ||___||
%
function FCC=MCCylCyl(N,L_H,D_H,varargin)

% Now to handle inputs- if not specified, assume we want 5 reflections
...
% ray = true(1,N);  % Currently "active" rays
out = false(1,N); % Terminated rays out of bounds (top/bottom planes)
faa = out;        % Terminated rays on the target (occlusion)

% We will also initialize some more arrays for memory happiness later:
L2  = zeros(1,N); % Occlusion helper variable
tc  = L2;
d2  = tc;
t1c = d2;
t1  = t1c;
...
MCW = rand(1,N); % Monte-Carlo RNG for psi distribution of bundles
...
if any(faa)
    FCC = sum(faa)/N;
    if sum(ray)~=0
        disp([num2str(sum(ray)) ' unterminated rays!'])
    end
elseif sum(ray)==N
    FCC = 0;
    disp('No terminated rays!')
else
    FCC = 0;
    disp('No rays on target!')
end
